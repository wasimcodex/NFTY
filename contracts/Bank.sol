...
contract Bank {
    ...
    event partialPaymentReturned(address applicant, uint256 credit, uint256 defaultedAmount, uint256 timestamp);
    ...
    public payable loanBalance(amount) {
    ...
    function auctionNFT(address applicant, uint256 lastBid, uint256 balanceAmount) public {
        require(block.timestamp > Loan[applicant].endTime, "Loan is not due");
        IERC721(Loan[applicant].nftContractAddress).approve(auctionContractAddress, Loan[applicant].tokenId);
        auctionContract.createNFTAuction(Loan[applicant].nftContractAddress, Loan[applicant].tokenId, balanceAmount, lastBid, 604800, true);
    ...
    function interest() public {
        ...
        for (uint i = 0; i < customers.length; i++) {
            if(balance[customers[i]] > 0){
                balance[customers[i]] = balance[customers[i]] + (balance[customers[i]] * 5 / 100);
            }
        }
    }

    function returnPartialPayment(address applicant, uint256 balanceAmount, uint256 sellingPrice) payable public {
        uint256 credit = sellingPrice - balanceAmount;
        payable(applicant).transfer(credit);
        emit partialPaymentReturned(applicant, credit, balanceAmount, block.timestamp);
    } 
